...
while True:
    ...
    for i in range(1, sheet_n+1):
        url = 'https://www.everyonepiano.com/pianomusic/%03d/%07d/%07d-w-b-%d.png'%(ceil(song_id/1000), song_id, song_id, i)
        print(url)
        response = requests.get(url, verify=False)
        sheets.append(Image.open(io.BytesIO(response.content)).convert('RGB'))
    print()

    sheets[0].save('./downloads/%s/%s-sheet.pdf'%(title.lower(), title.lower()), save_all=True, append_images=sheets[1:])
    print('downloaded sheet music pdf succesfully')
    print()

    numerics = []
    for i in range(1, sheet_n+1):
        url = 'https://www.everyonepiano.com/pianomusic/%03d/%07d/%07d-j-b-%d.png'%(ceil(song_id/1000), song_id, song_id, i)
        print(url)
        response = requests.get(url, verify=False)
        numerics.append(Image.open(io.BytesIO(response.content)).convert('RGB'))
    print()
    
    numerics[0].save('./downloads/%s/%s-jianpu.pdf'%(title.lower(), title.lower()), save_all=True, append_images=numerics[1:])
